...
class RRD(OffPolicyAlgorithm):
    ...
    def train_step(self, batch: Dict, step: int, total_steps: int) -> Dict:
        ...
        r_mean1 = torch.mean(reward, dim=1, keepdim=True)
        r_var_single = ((reward - r_mean1) ** 2).sum() / (batch["reward"].shape[0]-1)
        r_var = (r_var_single).mean()
        # print(reward.shape)
        # print(batch["reward"].unsqueeze(0).shape)
        # exit()

        

        r_loss = torch.nn.functional.mse_loss(reward.mean(), batch["reward"].unsqueeze(0).mean()).mean() + self.r_var * r_var
        replay_reward_loss = torch.nn.functional.mse_loss(reward, batch["reward"].unsqueeze(0)).mean()
